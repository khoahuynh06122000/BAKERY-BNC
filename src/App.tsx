...
export default function App() {
  ...
  const hasAdminPrivileges = isAdmin || isGodMode;
  ...
  if (user && isGuest && !workshop) {
    const handleJoin = () => {
      const targetWorkshop = inputXuong.trim();
      const isAdminLogin = targetWorkshop.toUpperCase() === "ADMIN";
      const match = WORKSHOPS.find(w => w.toLowerCase() === targetWorkshop.toLowerCase());
      if (!isAdminLogin && !match) {
        setLoginError("Tài khoản hoặc tên xưởng không tồn tại.");
        return;
      }
      if (isAdmin || inputPass === "123456") {
        if (isAdminLogin) {
          setIsGodMode(true);
          setWorkshop("ADMIN");
        } else if (match) {
    ...
    return (
      <div className="min-h-screen bg-[#080808] flex items-center justify-center p-4">
        <motion.div 
          ...
          className="w-full max-w-md bg-[#0c0c0c] border border-gold/10 p-6 md:p-10 rounded shadow-2xl relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-gold"></div>
          <div className="flex items-center gap-4 mb-8">
            <div className="w-10 h-10 bg-gold/10 flex items-center justify-center rounded border border-gold/20">
              <Warehouse className="text-gold w-5 h-5" />
            </div>
            <div>
              <h1 className="text-lg font-bold tracking-tight text-white uppercase italic">Đăng nhập</h1>
              <p className="text-[10px] text-white/40 uppercase tracking-widest leading-none mt-1">Hệ thống Bakery OS</p>
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-[10px] text-gold font-bold uppercase tracking-widest mb-1.5 ml-1">Tên Xưởng</label>
              <input 
                type="text"
                autoFocus
                value={inputXuong}
                onChange={(e) => { setInputXuong(e.target.value); setLoginError(""); }}
                placeholder="Ví dụ: Four Season"
            ...
            {!isAdmin && (
              <div>
                <label className="block text-[10px] text-gold font-bold uppercase tracking-widest mb-1.5 ml-1">Mật khẩu</label>
                <input 
                  type="password"
                  value={inputPass}
            ...
            {loginError && <p className="text-rose-500 text-[10px] font-medium uppercase tracking-wider">{loginError}</p>}
            <button
              onClick={handleJoin}
          ...
          <button 
            onClick={logout}
            className="mt-8 text-[10px] text-white/40 hover:text-white uppercase tracking-widest font-bold transition-all flex items-center gap-2"
          >
            <LogOut className="w-3 h-3" /> Thoát ứng dụng
          </button>
        </motion.div>
      </div>
    );
